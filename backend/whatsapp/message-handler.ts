...
type NamedCandidate = { id: string; nombre: string; score?: number; precioVenta?: number };
...
async function prepareOperation(
  ...
  if (needsClient(intent) && !entities.clientId) {
    ...
    if (resolved.status === 'none') {
      ...
      return {
        reply: `No encontré el cliente "${query}".\n¿Lo creo y sigo con la operación?\nRespondé SÍ para crear, otro nombre, o NO para cancelar.`,
  ...
  if (
    ...
    if (resolved.status === 'unique') {
      ...
      entities.productName = resolved.product.nombre;
...
async function handlePendingClarify(
  ...
  if (missingField === 'client') {
    ...
    entities.notes = [entities.notes, text.trim()].filter(Boolean).join(' · ');
  }

  return prepareOperation(businessId, phone, originalIntent, entities);
}
...
async function handlePendingSelection(
  ...
  if (!choiceMatch || !originalIntent || (!candidates.length && !(kind === 'product' && allowCreate && query))) {
    return {
      reply:
        kind === 'client'
          ? formatClientChoices(candidates, query)
          : formatProductChoices(candidates, query, { allowCreate }),
      intent: kind === 'client' ? SELECT_CLIENT_INTENT : SELECT_PRODUCT_INTENT,
      executed: false,
      businessId,
    };
  }
  ...
  // Opción "Crear nuevo" = último número
  if (kind === 'product' && allowCreate && query && index === candidates.length) {
    try {
      const created = await createCatalogProductFromWhatsapp(businessId, {
        nombre: query,
        precioVenta: unitPriceFromEntities(entities),
      });
      entities.productId = created.id;
      entities.productName = created.nombre;
      if (entities.amount == null && created.precioVenta > 0) {
        const qty = Math.max(1, Number(entities.quantity) || 1);
      ...
      return {
        reply: error instanceof Error ? error.message : 'No pude crear el producto.',
        intent: 'error',
        executed: false,
        businessId,
      };
  ...
  if (kind === 'client') {
    ...
    entities.productId = chosen.id;
    entities.productName = chosen.nombre;
    if (entities.amount == null && Number(chosen.precioVenta) > 0) {
      const qty = Math.max(1, Number(entities.quantity) || 1);
...
async function handleConfirmCreateClient(
  ...
  try {
    ...
    return {
      reply: error instanceof Error ? error.message : 'No pude crear el cliente.',
      intent: 'error',
      executed: false,
      businessId,
    };
...
async function handleConfirmCreateProduct(
  ...
  if (!CONFIRM_YES.test(text.trim())) {
    return {
      reply: `¿Guardo "${proposedName}" en el catálogo (sin stock)?\nSÍ = crear · NO = solo texto esta vez · cancelar = abortar.`,
      intent: CONFIRM_CREATE_PRODUCT,
      executed: false,
      businessId,
    };
...
export async function handleWhatsappMessage(
  ...
  if (!tenant) {
    ...
    const registerUrl = `${base}/registro?producto=whatsapp`;
    return {
      reply:
        `Hola 👋 Soy RiloBot.\n` +
  ...
  if (state?.pendingIntent && text) {
    ...
    if (state.pendingIntent === CLARIFY_INTENT) {
      return handlePendingClarify(tenant.businessId, phone, text, state.pendingPayload);
    }
    if (
  ...
  if (mediaId) {
    ...
    if (!image && !text) {
      return {
        reply:
          'Recibí la imagen pero no pude descargarla. Probá mandarla de nuevo con el texto del pedido.',
        intent: 'error',
        executed: false,
        businessId: tenant.businessId,
      };
  ...
  if (parsed.intent === 'help' || parsed.intent === 'greeting' || parsed.intent === 'unknown') {
    const result = await executeWhatsappCommand(tenant, parsed);
    return {
      reply: result.reply,
      intent: result.intent,
  ...
  return prepareOperation(tenant.businessId, phone, parsed.intent, entities);
}
