  ...
  template: `
    <div>
      <div class="flex items-center justify-between gap-2 mb-1">
        ...
        <span class="text-xs text-gray-400 dark:text-gray-500 tabular-nums">{{ photoCount }}/{{ maxPhotos }}</span>
      </div>
      <p *ngIf="showPrintHint" class="text-xs text-gray-500 dark:text-gray-400 mb-2">
        Se imprimen abajo del imprimible, en la misma hoja A4.
      </p>

      <p
        *ngIf="feedback"
        class="text-xs mb-2 rounded-lg px-2.5 py-1.5 border"
        [ngClass]="{
      ...
      <div class="flex flex-wrap items-center gap-2">
        <button
          *ngIf="canEdit"
          ...
          class="inline-flex items-center gap-2 px-3 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 text-sm font-semibold min-h-[40px] text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed shrink-0">
        ...
        <figure
          *ngFor="let photo of photos; trackBy: trackPhoto"
          class="group relative w-16 h-16 sm:w-[4.5rem] sm:h-[4.5rem] shrink-0 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden bg-gray-100 dark:bg-gray-900 shadow-sm">
          <button
            type="button"
          ...
          <button
            *ngIf="canEdit"
            ...
            [disabled]="uploadBusy"
            class="absolute top-0 right-0 inline-flex items-center justify-center w-8 h-8 min-h-[32px] min-w-[32px] rounded-bl-lg bg-black/60 text-white hover:bg-black/80 disabled:opacity-50 z-10 touch-manipulation"
            aria-label="Quitar foto"
            title="Quitar foto">
            <i-lucide name="x" class="w-3.5 h-3.5"></i-lucide>
          </button>
        </figure>

        <figure
          *ngFor="let pending of pendingPhotos; let index = index; trackBy: trackPending"
          class="group relative w-16 h-16 sm:w-[4.5rem] sm:h-[4.5rem] shrink-0 rounded-lg border border-dashed overflow-hidden shadow-sm"
